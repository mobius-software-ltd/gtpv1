...
public class CreateBearerRequestBearerContextToBeCreatedImpl extends BearerContextImpl implements CreateBearerRequestBearerContextToBeCreated
{
	...
	private Metric loadMetric;
	private List<APNAndRelativeCapacity> apnAndRelativeCapacity;
	...
	public void applyTLV(TLV2 tlv) throws GTPParseException 
	{
		...
		{		
			...
			case SEQUENCE_NUMBER:
				sequenceNumber=(SequenceNumber)tlv;
				break;
			case METRIC:
				loadMetric=(Metric)tlv;
				break;
			case APN_AND_RELATIVE_CAPACITY:
				if(apnAndRelativeCapacity==null)
					apnAndRelativeCapacity=new ArrayList<APNAndRelativeCapacity>();
				
				apnAndRelativeCapacity.add((APNAndRelativeCapacity)tlv);
				break;
			default:
	...
	public List<TLV2> getTLVs() throws GTPParseException 
	{
		...
		if(tft==null)
			throw new GTPParseException("TFT not set");
		...
		if(sequenceNumber==null)
			throw new GTPParseException("Sequence Number not set");
		
		output.add(sequenceNumber);
		
		if(loadMetric==null)
			throw new GTPParseException("Load Metric not set");
		
		output.add(loadMetric);
		
		if(apnAndRelativeCapacity!=null)
			output.addAll(apnAndRelativeCapacity);
	...
	@Override
	public SequenceNumber getLoadControlSequenceNumber() 
	{
		return sequenceNumber;
	}

	@Override
	public void setLoadControlSequenceNumber(SequenceNumber sequenceNumber) 
	{
		sequenceNumber.setInstance(0);
		this.sequenceNumber=sequenceNumber;
	}

	@Override
	public Metric getLoadMetric() 
	{
		return loadMetric;
	}

	@Override
	public void setLoadMetric(Metric loadMetric) 
	{
		loadMetric.setInstance(0);
		this.loadMetric=loadMetric;
	}

	@Override
	public List<APNAndRelativeCapacity> geAPNAndRelativeCapacity() 
	{
		return apnAndRelativeCapacity;
	}

	@Override
	public void setAPNAndRelativeCapacity(List<APNAndRelativeCapacity> apnAndRelativeCapacity) 
	{
		if(apnAndRelativeCapacity!=null)
			for(APNAndRelativeCapacity curr:apnAndRelativeCapacity)
				curr.setInstance(0);
		
		this.apnAndRelativeCapacity=apnAndRelativeCapacity;
	}
}